...
static int cs35l35_set_dai_fmt(struct snd_soc_dai *component_dai, unsigned int fmt)
{
...
static int cs35l35_set_pdm_dai_fmt(struct snd_soc_dai *component_dai,
	unsigned int fmt)
{
	struct snd_soc_component *component = component_dai->component;
	struct cs35l35_private *cs35l35 = snd_soc_component_get_drvdata(component);
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		...
		break;
	default:
		dev_err(component->dev, "PDM format is slave mode only\n");
		return -EINVAL;
	}
	...
	case SND_SOC_DAIFMT_PDM:
		break;
	default:
		dev_err(component->dev, "Not set to PDM format\n");
		return -EINVAL;
	}
	return 0;
}
				...
				int dir)
{
	...
	case 26000000:
		...
		break;
	default:
		dev_err(component->dev, "Invalid CLK Frequency\n");
		return -EINVAL;
	}
...
static struct snd_soc_component_driver soc_component_dev_cs35l35 = {
			      ...
			      const struct i2c_device_id *id)
{
	...
	cs35l35 = devm_kzalloc(&i2c_client->dev,
			       ...
			       GFP_KERNEL);
	if (!cs35l35) {
		dev_err(&i2c_client->dev, "could not allocate component\n");
		return -ENOMEM;
	}
	...
	ret =  snd_soc_register_component(&i2c_client->dev,
			&soc_component_dev_cs35l35, cs35l35_dai,
